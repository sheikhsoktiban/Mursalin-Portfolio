...
export function Contact() {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (name.length < 2 || !email.includes("@") || message.length < 8) return
    try {
      await addMessage({ name, email, category, subject, message })
      setOk(true)
  ...
  return (
    <section id="contact" className="py-20 bg-page">
      <div className="max-w-7xl mx-auto px-4">
        ...
        />
        <form onSubmit={onSubmit} className="max-w-3xl mx-auto bg-card border border-line rounded-3xl p-6 sm:p-10 space-y-4">
          {ok && <p className="text-xs text-muted font-mono">Message received. I’ll get back to you — or write {profile.email}.</p>}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <input name="subject" placeholder="Subject" className={fieldClass} />
          </div>
          <textarea name="message" required rows={6} placeholder="Hi Mursalin..." className={fieldClass} />
          <button className="w-full py-3.5 rounded-xl bg-accent text-accent-fg text-xs font-semibold">Send Message</button>
        </form>
      </div>
    </section>
  )
