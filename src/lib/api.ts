...
export const api = {
  ...
  async loadProjects(): Promise<Project[]> {
    if (supabase) {
      ...
      if (!error && data) return (data as Record<string, unknown>[]).map(asProject)
    }
  ...
  async starProject(id: string): Promise<number> {
    if (supabase) {
      const { data, error } = await supabase.rpc("increment_project_stars", { pid: id })
      if (error) throw error
  ...
  async loadGuestbook(): Promise<GuestNote[]> {
    if (supabase) {
      ...
      if (!error && data) return (data as Record<string, unknown>[]).map(asGuest)
    }
    return readLs(KEYS.guestbook, SEED_GUESTS)
  },

  async addGuest(note: Omit<GuestNote, "id" | "created_at">): Promise<GuestNote> {
    if (supabase) {
      const { data, error } = await supabase.from("guestbook").insert(note).select().single()
      if (error) throw error
    ...
    writeLs(KEYS.guestbook, [created, ...readLs(KEYS.guestbook, SEED_GUESTS)])
    return created
  },
  ...
  async deleteGuest(id: string): Promise<void> {
    ...
    writeLs(
      KEYS.guestbook,
      readLs(KEYS.guestbook, SEED_GUESTS).filter((g) => g.id !== id),
    )
  },
  ...
  async deleteMessage(id: string): Promise<void> {
    ...
    writeLs(
      KEYS.inbox,
      readLs(KEYS.inbox, SEED_INBOX).filter((m) => m.id !== id),
    )
  },
  ...
  async resetContent(): Promise<void> {
    if (supabase) {
      ...
      await supabase.from("profile").upsert(SEED_PROFILE)
